...
public class SplineSegment {
    ...
    public static List<Double> findRootsOfT(double lookup, NumArray4 coefficients) {
        ...
        List<Double> roots = CurveInterpolatorUtils.getCubicRoots(a, b, c, x);
        return roots.stream().filter(t -> t > eps && t <= 1 + eps).map(t -> CurveInterpolatorUtils.clamp(t, 0, 1)).collect(Collectors.toList());
    }
