...
public class CurveInterpolator {
    ...
    public double getTimeFromPosition(double position, boolean clampInput) {
        return this.curveMapper.getT(clampInput ? CurveInterpolatorUtils.clamp(position, 0, 1) : position);
    }

    public double getPositionFromTime(double t, boolean clampInput) {
        return this.curveMapper.getU(clampInput ? CurveInterpolatorUtils.clamp(t, 0, 1) : t);
    }

    public double getPositionFromLength(double length, boolean clampInput) {
        double l = clampInput ? CurveInterpolatorUtils.clamp(length, 0, this.length()) : this.length();
        return this.curveMapper.getU(l / this.length());
    }

    public double getLengthAt(double position, boolean clampInput) {
        return this.curveMapper.lengthAt(clampInput ? CurveInterpolatorUtils.clamp(position, 0, 1) : position);
    }
    ...
    public Vector getPointAtTime(double t, VectorType target) {
        t = CurveInterpolatorUtils.clamp(t, 0.0, 1.0);

        if(t == 0) {
            return CurveInterpolatorUtils.copyValues(this.points().get(0), null);
        } else if(t == 1) {
            return CurveInterpolatorUtils.copyValues(this.closed() ? this.points().get(0) : this.points().get(this.points().size() - 1), null);
        }
