...
import static com.lenerd.spotifyplus.module.lyrics.curveinterpolator.CurvyMath.magnitude;
import static com.lenerd.spotifyplus.module.lyrics.curveinterpolator.CurveInterpolatorUtils.getGaussianQuadraturePointsAndWeights;
...
public class NumericalCurveMapper extends AbstractCurveMapper {
        ...
        if(!((List<List<Double>>)this.cache.get("samples")).contains(idx)) {
            ...
            for(int i = 0; i < samples; ++i) {
                ...
                if(this.tension > 0.95) {
                    slope = CurveInterpolatorUtils.clamp(slope, -1, 1);
                }
    ...
    public double inverse(int idx, double len) {
        ...
        int i = Math.max(0, CurveInterpolatorUtils.binarySearch(len, lengths));
        double ti = i * step;
    ...
    public double getT(double u) {
        ...
        int i = CurveInterpolatorUtils.binarySearch(targetArcLength, arcLengths);
        int ti = i / (il - 1);
