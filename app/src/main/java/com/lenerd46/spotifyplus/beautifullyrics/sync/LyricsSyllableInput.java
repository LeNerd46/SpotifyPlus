...
final class LyricsSyllableInput extends EditText {
    ...
    int wordSelectionStart() {
        return originalOffset(getSelectionStart());
    }

    int wordSelectionEnd() {
        return originalOffset(getSelectionEnd());
    }
    ...
    void setSyllables(String word, SortedSet<Integer> cuts) {
        ...
        for (int i = 0; i < mapping.size(); i++) {
            originalOffsets[i] = mapping.get(i);
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (layout != null) {
            ...
            canvas.clipRect(getScrollX() + getCompoundPaddingLeft(), getScrollY(), getScrollX() + getWidth() - getCompoundPaddingRight(), getScrollY() + getHeight());
